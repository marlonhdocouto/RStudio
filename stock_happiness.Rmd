...
graph1<-sp_gdp%>%ggplot(aes(sp500.returns,gdp.growth))+geom_point()+theme_economist()+
...
graph2<-gdpx_spx%>%ggplot(aes(gdp.growth.t,sp500.returns.t1))+geom_point()+geom_smooth(method="lm")+
  theme_economist()+labs(title="Does GDP growth affects the stock market?",x="GDP growth at t",y="SP returns at t+1")
...
graph3<-sp_gdp_1%>%ggplot(aes(sp500.returns,gdp.growth))+geom_point()+theme_economist()+
  geom_smooth(method = "lm")+labs(title="Do S&P 500 returns linger in the economy?",x="SP returns at t",y="GDP growth at t+1")

graph4<-sp_gdp_2%>%ggplot(aes(sp500.returns,gdp.growth))+geom_point()+theme_economist()+
...
head(sp_quarterly.index,20)
head(gdp_quarterly.index,20)
...
graph5<-sp_gdp_real_scaled%>%ggplot(aes(sp500.scaled,gdp.scaled))+geom_point()+theme_economist()+
...
graph6<-sp_unem%>%ggplot(aes(sp500.returns,unemp.rate))+geom_point()+theme_economist()+geom_smooth(method = "lm")+
...
graph7<-sp_unem_2%>%ggplot(aes(sp500.returns,unemp.rate))+geom_point()+theme_economist()+geom_smooth(method = "lm")+
  labs(title="The effect of SP returns at t in the Unemployment rate at t+1",x="SP returns (t)",y="Unemployment (t+1)")
graph8<-sp_unem_3%>%ggplot(aes(sp500.returns,unemp.rate))+geom_point()+theme_economist()+geom_smooth(method = "lm")+
...
graph9<-sp_orders%>%ggplot(aes(sp500.returns,orders.growth))+geom_point()+theme_economist()+geom_smooth(method = "lm")+
  labs(title = "Effect of the stock market on Durable Goods Manufacturing",x="SP returns",y="Durable Goods growth")
graph9
```
...
graph10<-sp_orders_2%>%ggplot(aes(sp500.returns_t,orders.growth_t_1))+geom_point()+theme_economist()+geom_smooth(method="lm")+labs(title = "S&P500 returns at time t and Durable Goods Orders at t+1")
graph11<-sp_orders_3%>%ggplot(aes(orders.growth_t,sp500.returns_t_1))+geom_point()+theme_economist()+geom_smooth(method="lm")+labs(title = "Durable Goods Orders at time t and S&P500 returns at t+1")
...
cor(sp_7,sentiment)
graph12<-sp_sentiment%>%ggplot(aes(sp500.returns_endperiod,sentiment_begperiod))+geom_point()+theme_economist()+geom_smooth(method="lm")+labs(title="S&P500 returns and Consumer Sentiment",x="SP returns",y="Sentiment Levels")
graph12
...
colnames(sp_earn)<-c("sp500.returns","median.week.earning")
head(sp_earn)

cor(sp_earn$sp500.returns,sp_earn$median.week.earning)
graph13<-sp_earn%>%ggplot(aes(sp500.returns,median.week.earning))+geom_point()+theme_economist()+geom_smooth(method="lm")+
  labs(title = "S&P500 and Median Income",x="SP returns",y="Median wages")
graph13
```
...
What about household debt? It looks like debt has been through the roof and indebted persons face financial uncertainty; therefore, we can assume them to be more unhappy. Is there relationship between stock market returns and the levels of debt in the economy?

The measure for debt that was downloaded from the federal reserve website (https://www.federalreserve.gov/releases/g19/HIST/cc_hist_sa_levels.html) and it is the total revolving debt in the economy.

To find about revolving debt: https://www.experian.com/blogs/ask-experian/credit-education/score-basics/credit-utilization-rate/

```
The first observation is on Jan/1/1968. This is possibly due to the fact that credit cards became widely introduced into the economy in the 50s and revolving debt only became widely available with BankAmericard (now Visa) in 1966. You can read more about this story here: https://www.creditcards.com/credit-card-news/history-of-credit-cards/
```

I will calculate the change in revolving debt and find correlations between totals and monthly changes
...
rownames(debt)<-debt[,1]
debt<-debt[90:nrow(debt),2:4]

rate_change<-c(0)
for(i in 2:nrow(debt)){
  rate<-debt[i,3]/debt[(i-1),2]
  rate<-round(rate*100,3)
  rate_change<-append(rate_change,rate)
}
debt<-cbind(debt,rate_change)
debt<-debt[2:nrow(debt),]
head(debt)

sp_9<-monthlyReturn(GSPC)
sp_9<-sp_9[1:(nrow(sp_9)-3),]

debt_scaled<-scale(debt$`Revolving _monthly`)
sp_9_scaled<-scale(sp_9$monthly.returns)
sp_debt_scaled<-as.data.frame(cbind(sp_9,debt_scaled))
colnames(sp_debt_scaled)<-c("sp500.scaled","debt.scaled")
cor(debt_scaled,sp_9_scaled)
graph14<-sp_debt_scaled%>%ggplot(aes(sp500.scaled,debt.scaled))+geom_point()+geom_smooth(method="lm")+
  theme_economist()+labs(title = "The relationship between the S&P500 and Revolving Debt",x="SP returns (scaled)",
                         y="Debt levels (scaled)")
graph14
...
There is a 10% correlation between S&P returns and revolving debt in the economy. This might be correlated to what I saw before in relation to consumer confidence. As consumers get more confident about the economy and their prospects (probably due to rises in the stock market), they spend more and go into more debt. Whether this is good or bad is really out of the scope of this discussion. 
...
graph15<-inequality_gini3%>%ggplot(aes(log(gini),log(Price)))+geom_point()+
  geom_smooth(method="lm")+theme_economist()+labs(title="Correlation between S&P and Inequality",x="SP returns (log)",
                                       y="Gini Index (log)")
graph15
```
...
head(inequality_gini4)

quintile.growth<-NULL
growth.vector<-c(10)
for (i in 1:ncol(inequality_gini4)){
...
colnames(quintile.growth)<-c("low.quintile","second.quint","mid.quint","fourth.quint","high.quint","sp.income")
...
graph18<-ggplot(data=wealth.growth,aes(x=Year,y=income))+geom_line(aes(color=factor(Category)))+
  theme_economist()+geom_step(data = sp.prices.90,aes(x=Year,y=sp.income),size=1)+
  labs(title="Growth of $10,000 under each bracket and Stock Market",x="Year",
                   y="Income (in thousands)",color=" ")
graph18
